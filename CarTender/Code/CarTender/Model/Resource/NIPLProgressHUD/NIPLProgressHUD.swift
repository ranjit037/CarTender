...
public enum NotificationName : String {
    case NIPLProgressHUDDidReceiveTouchEvent, NIPLProgressHUDDidTouchDownInside, NIPLProgressHUDWillDisappear, NIPLProgressHUDDidDisappear, NIPLProgressHUDWillAppear, NIPLProgressHUDDidAppear, NIPLProgressHUDStatusUserInfoKey
    public func getNotificationName() -> Notification.Name {
        return Notification.Name.init(self.rawValue)
    }
}

public enum NIPLProgressHUDStyle : Int {
    case light
    case dark
    case custom
}

public enum NIPLProgressHUDMaskType : Int {
    case none = 1
    case clear
    case black
    case gradient
...
public enum NIPLProgressHUDAnimationType : Int {
    case flat
    case native
}

private let NIPLProgressHUDParallaxDepthPoints : CGFloat = 10.0
private let NIPLProgressHUDUndefinedProgress : CGFloat = -1
private let NIPLProgressHUDDefaultAnimationDuration: CGFloat = 0.15
private let NIPLProgressHUDVerticalSpacing: CGFloat = 12.0
private let NIPLProgressHUDHorizontalSpacing: CGFloat = 12.0
private let NIPLProgressHUDLabelSpacing: CGFloat = 8.0

public class NIPLProgressHUD : UIView {
    
    static var isNotAppExtension = true

    private var  defaultStyle = NIPLProgressHUDStyle.light
    private var defaultMaskType = NIPLProgressHUDMaskType.none
    private var defaultAnimationType = NIPLProgressHUDAnimationType.flat
    ...
    private var fadeInAnimationDuration: TimeInterval = TimeInterval(NIPLProgressHUDDefaultAnimationDuration)
    private var fadeOutAnimationDuration: TimeInterval = TimeInterval(NIPLProgressHUDDefaultAnimationDuration)
    ...
    private override init(frame: CGRect) {
        ...
        accessibilityIdentifier = "NIPLProgressHUD"
    ...
    private static let sharedView : NIPLProgressHUD = {
        var localInstance : NIPLProgressHUD?
        if Thread.current.isMainThread {
            if NIPLProgressHUD.isNotAppExtension {
            if let window = UIApplication.shared.delegate?.window {
                localInstance = NIPLProgressHUD.init(frame: window?.bounds ?? CGRect.zero)
            } else {
                localInstance = NIPLProgressHUD()
                }
            }
            else {
            localInstance = NIPLProgressHUD.init(frame: UIScreen.main.bounds)
            }
        } else {
    ...
    @objc private func controlViewDidReceiveTouchEvent(_ sender: Any?, for event: UIEvent?) {
        NotificationCenter.default.post(name: NotificationName.NIPLProgressHUDDidReceiveTouchEvent.getNotificationName(), object: self, userInfo: notificationUserInfo())

        if let touchLocation = event?.allTouches?.first?.location(in: self) {
            if getHudView().frame.contains(touchLocation) {
                NotificationCenter.default.post(name:
                    NotificationName.NIPLProgressHUDDidTouchDownInside.getNotificationName(), object: self, userInfo: notificationUserInfo())
            }
        }
    ...
    func notificationUserInfo() -> [String : Any]? {
        if let statusText = getStatusLabel().text {
            return [NotificationName.NIPLProgressHUDStatusUserInfoKey.rawValue: statusText]
        }
        return nil
    }
    ...
    @objc private func fadeIn(_ object: AnyObject?) {
        ...
        if getBackGroundView().alpha != 1.0 {
            NotificationCenter.default.post(name: NotificationName.NIPLProgressHUDWillAppear.getNotificationName(), object: self, userInfo: notificationUserInfo())
            ...
            let completionBlock : () -> Void = {
                ...
                NotificationCenter.default.post(name: NotificationName.NIPLProgressHUDDidAppear.getNotificationName(), object: self, userInfo: self.notificationUserInfo())
    ...
    @objc private func positionHUD(_ notification: Notification? = nil) {
        ...
        if NIPLProgressHUD.isNotAppExtension == false {
...
//                self.frame = containerView!.frame
                ...
                // If NIPLProgressHUD is used inside an app extension add it to the given view
                if viewForExtension != nil {
    ...
    private func show(image: UIImage, status: String?, duration: TimeInterval) {
        OperationQueue.main.addOperation({ [weak self] in
            ...
            strongSelf.progress = Float(NIPLProgressHUDUndefinedProgress)
            strongSelf.cancelRingLayerAnimation()
            strongSelf.cancelIndefiniteAnimatedViewAnimation()
    ...
    private func dismissWithDelay(_ delay: TimeInterval, completion: (() -> Void)?) {
        OperationQueue.main.addOperation({ [weak self] in
            guard let strongSelf = self else { return }
                ...
                NotificationCenter.default.post(name: NotificationName.NIPLProgressHUDWillDisappear.getNotificationName(), object: nil, userInfo: strongSelf.notificationUserInfo())
                ...
                let completionBlock: (() -> Void) = {
                    ...
                    if strongSelf.getBackGroundView().alpha == 0.0 {
                        ...
                        // Post notification to inform user
                        //NIPLProgressHUDDidDisappearNotification
                        NotificationCenter.default.post(name: NotificationName.NIPLProgressHUDDidDisappear.getNotificationName(), object: strongSelf, userInfo: strongSelf.notificationUserInfo())
    ...
    private func updateHUDFrame() {
        ...
        hudWidth = CGFloat(NIPLProgressHUDHorizontalSpacing + max(labelWidth, contentWidth) + NIPLProgressHUDHorizontalSpacing)

        // |-spacing-content-(labelSpacing-label-)spacing-|
        hudHeight = CGFloat(NIPLProgressHUDVerticalSpacing) + labelHeight + contentHeight + CGFloat(NIPLProgressHUDVerticalSpacing)
        if ((getStatusLabel().text != nil) && (imageUsed || progressUsed )) {
            // Add spacing if both content and label are used
            hudHeight += CGFloat(NIPLProgressHUDLabelSpacing)//8 [80]
        }
        ...
        var centerY: CGFloat
        if getStatusLabel().text != nil {
            let yOffset = max(NIPLProgressHUDVerticalSpacing, (minimumSize.height - contentHeight - CGFloat(NIPLProgressHUDLabelSpacing) - labelHeight) / 2.0)//12
        ...
        if CGFloat(progress) != NIPLProgressHUDUndefinedProgress {
        ...
        if imageUsed || progressUsed {
            if imageUsed {
                centerY = getImageView().frame.maxY + NIPLProgressHUDLabelSpacing + labelHeight / 2.0
            } else {
                centerY = getIndefiniteAnimatedView().frame.maxY + NIPLProgressHUDLabelSpacing + labelHeight / 2.0
            }
        } else {
            centerY = getHudView().bounds.midY
        }
    ...
    private func updateMotionEffect(forXMotionEffectType xMotionEffectType: UIInterpolatingMotionEffect.EffectType, yMotionEffectType: UIInterpolatingMotionEffect.EffectType) {
        ...
        effectX.minimumRelativeValue = -NIPLProgressHUDParallaxDepthPoints
        effectX.maximumRelativeValue = NIPLProgressHUDParallaxDepthPoints
        
        let effectY = UIInterpolatingMotionEffect(keyPath: "center.y", type: yMotionEffectType)
        effectY.minimumRelativeValue = -NIPLProgressHUDParallaxDepthPoints
        effectY.maximumRelativeValue = NIPLProgressHUDParallaxDepthPoints
...
extension NIPLProgressHUD {
    
    public class func set(defaultStyle style: NIPLProgressHUDStyle) {
    ...
    public class func set(defaultMaskType maskType: NIPLProgressHUDMaskType) {
        sharedView.defaultMaskType = maskType
    }
    
    public class func set(defaultAnimationType type: NIPLProgressHUDAnimationType) {
...
//        sharedView.defaultStyle = .custom
    ...
    public class func set(viewForExtension view: UIView) {
        NIPLProgressHUD.isNotAppExtension = false
    ...
    public class func show(withStatus status: String?) {
        show(progress: NIPLProgressHUDUndefinedProgress, status: status)
    }
...
extension NIPLProgressHUD {
    ...
    private func loadImageBundle(named imageName:String) -> UIImage? {
        let imageBundle = Bundle.init(for: NIPLProgressHUD.self)
